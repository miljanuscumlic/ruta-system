		...
		else
		{
			...
			case 3:
				XMLGregorianCalendar cal = party.getPartyLegalEntity().size() == 0 ? null :
					InstanceFactory.getPropertyOrNull(party.getPartyLegalEntity().get(0).getRegistrationDate(), RegistrationDateType::getValue);
				if(cal != null)
				{
					LocalDate date = InstanceFactory.getLocalDate(party.getPartyLegalEntity().get(0).getRegistrationDate().getValue());
