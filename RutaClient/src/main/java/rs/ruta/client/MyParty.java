...
public class MyParty extends BusinessParty
{
	...
	public void processDocBoxInvoice(InvoiceType invoice) throws DetailException, InterruptedException
	{
		...
		synchronized(corr) //defence against ill arrival of multiple documents of the same type at the same time for a particular correspondence
		{
			...
			{
				throw new DetailException("Invoice " + invoice.getIDValue() + " has been already received and processed. Try resending your most recently created document if it is the last document of the correspondence.");  
			}
			final RutaProcess process =  (RutaProcess) corr.getState();
					...
					process.getState() instanceof CustomerReceiveInvoiceState)
			{
				...
				throw new DetailException("Invoice " + invoice.getIDValue() + 
						" does not belong to the current state of the correspondence. Try resending your most recently created document if it is the last document of the correspondence."); 
		}
	}
	...
	public void processDocBoxOrderResponse(OrderResponseType orderResponse) throws DetailException, InterruptedException
	{
		...
		synchronized(corr) //defence against ill arrival of multiple documents of the same type at the same time for a particular correspondence
		{
			...
			if(existingCorr != null && existingCorr.getIdValue().equals(corr.getIdValue()))
			{
				throw new DetailException("Order Response " + orderResponse.getIDValue() + " has been already received and processed. Try resending your most recently created document if it is the last document of the correspondence.");  
			}
			final RutaProcess process =  (RutaProcess) corr.getState();
	...
	public void processDocBoxOrderChange(OrderChangeType orderChange) throws DetailException, InterruptedException
	{
		...
		synchronized(corr) //defence against ill arrival of multiple documents of the same type at the same time for a particular correspondence
		{
			...
			if(existingCorr != null && existingCorr.getIdValue().equals(corr.getIdValue()))
			{
				throw new DetailException("Order Change " + orderChange.getIDValue() + " has been already received and processed. Try resending your most recently created document if it is the last document of the correspondence.");  
			}
			final RutaProcess process =  (RutaProcess) corr.getState();
	...
	public void processDocBoxDocumentReceipt(DocumentReceipt documentReceipt) throws DetailException
	{
		...
		synchronized(corr)
		{
			...
			{
				corr.updateOutOfSyncStatus(true);
				throw new DetailException("Document Receipt has been rejected. Correspondences of both client sides are not synchronized with each other!"); 
			}
		}
	}
	...
	public void processDocBoxPartnershipBreakup(PartnershipBreakup breakup) throws DetailException
	{
		...
		if(partnerBreaker != null)
			...
			throw new DetailException("Party witd ID " + correspondentID + " is not a Business Partner of My Party.");  
